...
@section('content')
    <main>
        ...
        <section class="page__title p-relative d-flex align-items-center"
                 data-background="{{$product->getFirstMediaUrl('main_image')}}"
                            ...
                            <div class="col-xl-6 col-lg-6">
                                <div class="product__modal-box d-flex">
                                    ...
                                    <div class="tab-content mb-20" id="product-detailsContent">
                                        <div class="tab-pane fade active show" id="pro-two" role="tabpanel"
                                            ...
                                            <div class="nav nav-tabs" id="product-details" role="tablist">
                                                ...
                                                @foreach($product->getMedia('images') as $image)
                                                    <a class="nav-item nav-link {{$loop->iteration==1 ? 'active' : ''}}"
                                                       id="pro-{{$image->id}}-tab" data-toggle="tab"
                                                       href="#pro-{{$image->id}}" role="tab"
                                                       aria-controls="pro-{{$image->id}}" aria-selected="true">
                                    ...
                                    <div class="tab-content mb-20" id="product-detailsContent">
                                        @foreach($product->getMedia('images') as $image)
                                            <div class="tab-pane fade {{$loop->iteration==1 ? 'show active' : ''}}"
                                                 id="pro-{{$image->id}}" role="tabpanel"
                                                 aria-labelledby="pro-{{$image->id}}-tab">
                                                <div class="product__modal-img product__thumb w-img">
                                                    <img src="{{$image->getUrl()}}" alt="">
                        ...
                        <div class="col-xl-6 col-lg-6">
                            <div class="product__modal-content product__modal-content-2">
                                ...
                                @if($is_product_in_cart)
                                    ...
                                    <div>
                                        <a class="add-cart-btn  mb-20"
                                           data-id="{{$product->id}}">{{@trans('content.in_cart')}}</a>
                                    </div>
                                @else
                                    {{-- $product->id cookie-də mövcud deyilsə --}}
                                    {{-- Düyməni göstərir --}}
                                    <div>
                                        <a class="add-cart-btn add-cart mb-20" data-id="{{$product->id}}"
                                           style="cursor: pointer">{{@trans('content.addToCart')}}</a>
                                    </div>
                                @endif
                            ...
                            <div class="product__wrapper mb-60">
                                <div class="product__thumb">
                                    <a href="{{route('product',$related_product->id)}}" class="w-img">
                                        <div class="productImg"
                                             style="background-image: url({{$related_product->getFirstMediaUrl('main_image','thumb')}})"></div>
                                    </a>
                                ...
                                <div class="product__content p-relative">
                                    <div class="product__content-inner">
                                        <h4>
                                            <a href="{{route('product',$related_product->id)}}">{{$related_product->name}}</a>
                                        </h4>
                                        @if(auth()->check() && auth()->user()->discount_percent!=0)
